...
public abstract class AbstractParallelPipe<IN, OUT, V> extends
     ...
     * @throws Exception
     */
    protected abstract OUT combinesResults(List<Future<V>> subTaskResults)
            throws Exception;
    ...
    @Override
    public OUT doProcess(final IN input) throws PipeException {
        OUT out = null;
        try {
            out = combinesResults(invokeParallel(buildTasks(input)));
        } catch (Exception e) {
