...
public class TestSimpleMovingAverage { 
    ...
    public static void main(String[] args) {
        ...
        for (int windowSize : allWindowSizes) {
            SimpleMovingAverageUsingArray smasa = new SimpleMovingAverageUsingArray(windowSize);
            THE_LOGGER.info("windowSize = " + windowSize);
            for (double x : testData) {
                smasa.addNewNumber(x);
                THE_LOGGER.info("Next number = " + x + ", SMA = " + smasa.getMovingAverage());
            }
            THE_LOGGER.info("---");
        }
    }
