    ...
    public static ResultScanner getScanner(String tableName, String startKey, String stopKey) {
        try (Table table = HBaseConn.getTable(tableName)) {
            ...
            results.forEach(result -> {
                ...
                System.out.println("basic:name == " + Bytes.toString(result.getValue(Bytes.toBytes("cf1"), Bytes.toBytes("name"))));
            });
            return results;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static ResultScanner getScanner(String tableName, String startKey, String stopKey, FilterList filterList) {
        try (Table table = HBaseConn.getTable(tableName)) {
            ...
            results.forEach(result -> {
                ...
                System.out.println("basic:type == " + Bytes.toString(result.getValue(Bytes.toBytes("cf1"), Bytes.toBytes("type"))));
                System.out.println("basic:dest == " + Bytes.toString(result.getValue(Bytes.toBytes("cf1"), Bytes.toBytes("dest"))));
                System.out.println("basic:time == " + Bytes.toString(result.getValue(Bytes.toBytes("cf1"), Bytes.toBytes("time"))));
            });
            return results;
        } catch (Exception e) {
            e.printStackTrace();
